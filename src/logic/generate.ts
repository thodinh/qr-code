...
import { DOMMatrix, Image, ImageData, createCanvas } from 'canvas'
...
export async function generateQRCode(state: QRCodeGeneratorState) {
  ...
  if (state.effect === 'crystalize') {
    ...
    ctx.putImageData(newData as ImageData, 0, 0)
  }
    ...
    const newData2 = effects.liquidify(
      newData1 as ImageData,
      state.effectLiquidifyRadius,
      state.effectLiquidifyThreshold,
      state.lightColor,
      state.darkColor,
    )
    ctx.putImageData(newData2 as ImageData, 0, 0)
  }
  ...
  async function applyBackground() {
    ...
    if (state.backgroundImage) {
      if (state.backgroundImage.startsWith('#')) {
        ...
        await new Promise(resolve => img.onload = resolve as any).then()
