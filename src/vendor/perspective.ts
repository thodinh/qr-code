...
export default class Perspective {
  ...
  constructor(ctxd: CanvasRenderingContext2D, target: ImageData) {
    ...
    const cvso = createCanvas(target.width, target.height)
  ...
  draw(q: Quadrilateral) {
    ...
    if (base_index % 2 == 0) {
      ...
      for (let y = 0; y < oh; y += step) {
        ...
        const transMatrixl = [1, 0, 0, 1, 0, -y]
        //@ts-ignore
        ctxl.setTransform(...transMatrixl);
      ...
      for (let x = 0; x < ow; x += step) {
        ...
        const transMatrix = [1, 0, 0, 1, -x, 0]
        //@ts-ignore
        ctxl.setTransform(...transMatrix);
        ctxl.drawImage(ctxo.canvas, 0, 0);
        //
        ctxt.translate(sx, sy);
        ctxt.rotate(ag);
        ...
        ctxt.drawImage(cvsl, 0, 0);
        //
        const transMatrixl = [1, 0, 0, 1, 0, 0]
        //@ts-ignore
        ctxt.setTransform(...transMatrixl);
      }
    }
